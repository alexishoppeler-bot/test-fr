...
function melangerOptions(opts = []) {
  const jnsp = [];
  const autres = [];

  opts.forEach(opt => {
    if (opt === "Je ne sais pas") jnsp.push(opt);
    else autres.push(opt);
  });

  return [...melangerTableau(autres), ...jnsp];
}
...
function affQuestion() {
  ...
  const optsAffichees = Array.isArray(q.opts) ? melangerOptions(q.opts) : [];
  ...
  switch(q.type) {
    ...
    case "demele":
      const melange = melangerTableau(q.mots);
      html += `
...
function affResultat() {
  ...
  const emailParams = {
    ...
    reponses_html: repDetail.map((r, i) =>
      `<strong>Q${String(i + 1).padStart(2, "0")} - ${r.question_id}</strong><br>` +
      `Choisie : ${_txt(r.reponse_utilisateur)}<br>` +
      `Correcte : ${_txt(r.reponse_correcte)}<br>` +
      `Statut : ${r.correct ? "Correcte" : "Incorrecte"}`
    ).join("<br><br>")
  };
...
function _nomPdfResultats(params) {
  const base = `${params.prenom || "candidat"}_${params.nom || "resultats"}_${params.niveau || "niveau"}`;
  return base
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
...
async function _genererPdfReponses(params) {
  const jspdfNs = window.jspdf;
  if (!jspdfNs?.jsPDF) throw new Error("jsPDF non charge");
  ...
  const addLines = (text, size = 11, gap = 6) => {
  ...
  doc.setFontSize(18);
  doc.text("Resultats du test de francais", margin, y);
  y += 10;

  addLines(`Candidat : ${params.prenom || "-"} ${params.nom || ""}`.trim(), 11, 6);
  addLines(`Email : ${params.email || "-"}`, 11, 6);
  addLines(`Niveau estime : ${params.niveau || "-"} | Score : ${params.score || "-"}% | Duree : ${params.duree || "-"}`, 11, 6);
  addLines(`Date : ${params.date_test || "-"} ${params.heure_test || ""}`.trim(), 11, 8);

  doc.setFont("helvetica", "bold");
  doc.setFontSize(12);
  doc.text("Reponses choisies", margin, y);
  y += 8;

  repDetail.forEach((r, idx) => {
    if (y > pageH - 40) {
      doc.addPage();
      y = margin;
    }
    doc.setFont("helvetica", "bold");
    doc.setFontSize(12);
    doc.text(`Q${String(idx + 1).padStart(2, "0")} - ${r.question_id}`, margin, y);
    y += 6;

    addLines(`Reponse choisie : ${_txt(r.reponse_utilisateur)}`, 10, 5);
    addLines(`Bonne reponse : ${_txt(r.reponse_correcte)}`, 10, 5);
    addLines(`Statut : ${r.correct ? "Correcte" : "Incorrecte"}`, 10, 7);
  });

  const blob = doc.output("blob");
  return new File([blob], _nomPdfResultats(params), { type: "application/pdf" });
}

async function _buildEmailForm(params, toEmail) {
  const form = document.createElement("form");
  form.style.display = "none";

  Object.entries({ ...params, to_email: toEmail }).forEach(([key, value]) => {
  ...
  const pdfFile = await _genererPdfReponses(params);
  const fileInput = document.createElement("input");
  fileInput.type = "file";
  fileInput.name = "results_pdf";
...
function _envoyerEmailAvecPdf(params, toEmail, msgEl) {
  console.log("PDF mail ->", toEmail);

  const ej = window._emailjs;
  if (!ej || typeof emailjs === "undefined") {
    if (msgEl) { msgEl.textContent = "EmailJS non charge."; msgEl.style.color = "var(--warning)"; }
    return Promise.reject("EmailJS non charge");
  }

  if (msgEl) { msgEl.textContent = "Envoi de l'e-mail avec PDF..."; msgEl.style.color = ""; }

  return _buildEmailForm(params, toEmail)
    .then(form => emailjs.sendForm(ej.serviceId, ej.templateId, form).finally(() => form.remove()))
    .catch(err => {
      console.warn("Envoi avec PDF impossible, repli sans piece jointe.", err);
      return emailjs.send(ej.serviceId, ej.templateId, { ...params, to_email: toEmail });
    })
    .then(() => {
      if (msgEl) { msgEl.textContent = "Resultats envoyes."; msgEl.style.color = "var(--success)"; }
    })
    .catch(err => {
      const detail = err?.text || err?.message || JSON.stringify(err);
      console.error("EmailJS erreur:", err);
...
function envoyerEmailConfirm() {
  ...
  _envoyerEmailAvecPdf(_lastEmailParams, "ecole.francophone@icloud.com", msg)
    .then(() => {
...
function envoyerEmailTest() {
  ...
  _envoyerEmailAvecPdf(_lastEmailParams, "alexis.hoppeler@gmail.com")
